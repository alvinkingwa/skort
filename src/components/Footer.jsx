import { Flame, UserPlus, Instagram, Twitter, Mail, Shield, FileText, HelpCircle, Home } from "lucide-react";
import { brand, brandDark, brandHover, brandText } from "../theme.js";

const Footer = ({ onJoinCreator, onTabChange }) => (
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-6 mb-8">
        ...
        <div>
          ...
          <ul className="space-y-2">
            <li>
              <button onClick={() => onTabChange?.("creators")} className="flex items-center gap-1.5 text-xs text-zinc-600 hover:text-white transition-colors">
                <Flame className="w-3 h-3" /> Creators
              </button>
            </li>
            <li>
              <button onClick={() => onTabChange?.("stays")} className="flex items-center gap-1.5 text-xs text-zinc-600 hover:text-white transition-colors">
